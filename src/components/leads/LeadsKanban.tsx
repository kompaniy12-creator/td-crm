...
export function LeadsKanban({ leads, onLeadClick }: { leads: Lead[]; onLeadClick?: (lead: Lead) => void }) {
  ...
  return (
    <div className="flex h-full gap-3 overflow-x-auto p-4 pb-6">
      {columns.map((col) => {
        ...
        return (
          <div key={col.status} className="flex w-72 flex-shrink-0 flex-col rounded-xl bg-gray-50 border border-gray-200">
            <div className="flex items-center justify-between px-3 py-2.5 border-b border-gray-200">
              <div className="flex items-center gap-2">
                <h3 className={cn('text-sm font-semibold', col.color)}>{col.label}</h3>
                <span className={cn('rounded-full px-2 py-0.5 text-xs font-medium', col.bgColor, col.color)}>{colLeads.length}</span>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-2 space-y-2">
              {colLeads.length === 0 ? (<div className="flex items-center justify-center py-8 text-xs text-gray-400">Нет лидов</div>) : (colLeads.map((lead) => (<LeadCard key={lead.id} lead={lead} onClick={() => onLeadClick?.(lead)} />)))}
            </div>
          </div>
        )
      })}
