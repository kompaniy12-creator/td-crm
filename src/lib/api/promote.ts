...
/**
 * Client-side port of the former `/api/leads/[id]/promote-to-deal` route.
 * Promotes a Lead into a sales Deal using the browser Supabase client.
 */
export async function promoteLeadToDeal(
  ...
  if (!contactId) {
    const { data: newContact, error: cErr } = await supabase
      ...
      .single()
    if (cErr || !newContact) {
      return { error: 'Не удалось создать контакт' }
    }
  ...
  if (dErr || !newDeal) {
    return { error: 'Не удалось создать сделку' }
  }
