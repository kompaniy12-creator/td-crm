...
function Inner() {
  ...
  useEffect(() => {
    ...
    async function run() {
      if (errParam) { setError(errParam); setPhase('error'); return }
      if (!code || !state) { setError('Facebook вернул пустой код или state'); setPhase('error'); return }
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow">
        ...
        {phase === 'done' && (
          <div className="flex items-center gap-3 text-green-700">
            ...
            <div>
              <p className="font-medium">Facebook авторизован</p>
              <p className="text-xs text-gray-500">Воркер завершит обмен токенами и подключит страницы. Возвращаю…</p>
            </div>
          </div>
        )}
        {phase === 'error' && (
